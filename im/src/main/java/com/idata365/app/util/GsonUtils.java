...
public class GsonUtils {
	 ...
	 * @return
	 */
	public static <T> T fromJson2(String json, TypeToken<T> typeToken) {
		...
		}.getType(), new MapTypeAdapter2()).create();
...
//	}
	...
	public static class MapTypeAdapter2 extends TypeAdapter<Object> {
		...
		public Object read(JsonReader in) throws IOException {
			...
			case NUMBER:
				...
				// 判断数字是否为整数值
				if(Integer.MIN_VALUE<dbNum && dbNum<Integer.MAX_VALUE){
				...
				else if(Long.MIN_VALUE<dbNum && dbNum<Long.MAX_VALUE){
					long lngNum = (long) dbNum;
					if (dbNum == lngNum) {
						return lngNum;
					} else {

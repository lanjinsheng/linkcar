...
//		System.out.println(new Date().getTime());
	...
	public void run() {  
		...
		if(threadPool.getActiveCount()>=10) return;
		synchronized (lock){
		if(pd){
			pd=false;
			...
			for(UploadDataStatus us:list) {
				threadPool.execute(new DatasDealTask(us.getUserId(),us.getHabitId(),us.getTaskFlag(),us.getHadSensorData(),us.getId(),us.getDealTimes()));
			}
			pd=true;
		}

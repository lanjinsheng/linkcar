...
<script>
  ...
  function doLogin(){
				$("#ff").ajaxSubmit({
					type:"post",
					url:"<%=contextPath%>/r/manage/login/userLogin",
					success:function(rtJson){
						if (rtJson.rtState == '0'){
							window.location.href="<%=contextPath%>/main.jsp";
				} else {
					$.messager.alert("操作提示！", rtJson.rtMsrg);
				}
			}
		});
	}
			...
			<DIV
				...
				<P style="margin: 0px 35px 20px 45px;">
					<SPAN style="float: center; ">
						<A style="background: rgb(0, 142, 173); padding: 10px 100px; border-radius: 4px; border: 1px solid rgb(26, 117, 152); border-image: none; color: rgb(255, 255, 255); font-weight: bold;"
						href="javascript:void(0);"  onclick="doLogin();">登录</A> </SPAN>
				</P>
			</DIV>
		</DIV>
	</form>
